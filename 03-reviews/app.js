...
{let currentItem = reviews[idx]; 

img.src = currentItem.img; 
author.textContent = currentItem.name; 
job.textContent = currentItem.job; 
info.textContent = currentItem.text; }
//on load set the first person
window.addEventListener('DOMContentLoaded', function(){
  setPerson(idx);
})
 

//get the next person when the nextbtn is pressed 
nextBtn.addEventListener('click', ()=>{
  setPerson((++idx)%reviews.length); 
}); 

//get the prev person when the prevBtn is pressed 
